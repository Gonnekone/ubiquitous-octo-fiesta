...
type RefreshTokenProvider interface {
	...
	TokenExists(ctx context.Context, guid string) (string, error)
}
...
func New(log *slog.Logger, refreshTokenProvider RefreshTokenProvider, jwtService *jwt.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		refreshTokenHash, err := refreshTokenProvider.TokenExists(r.Context(), accessGuid)
		if err != nil {
			log.Error("failed to check if refresh token is exists", sl.Err(err))
